...
def start_training(inputs, enc_units, generator, encoder, par_enc_model, gen_enc_model, discriminator, gen_disc_alter):
  ...
  for step, (x_batch_train, y_batch_train) in enumerate(zip(input_tokens, target_tokens)):
      ...
      with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
          ...
          par_enc_real_state_x = par_enc_model(unrolled_x, training = gen_disc_alter)
          gen_real_enc_state_y = gen_enc_model(real_y, training = gen_disc_alter)
          gen_enc_fake_state_x = gen_enc_model(generated_logits, training = gen_disc_alter)
